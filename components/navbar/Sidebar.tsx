...
export default function Sidebar({ isCollapsed, setIsCollapsed }: SidebarProps) {
  ...
  const [isAdGroupMenuOpen, setIsAdGroupMenuOpen] = useState(false)
  ...
  useEffect(() => {
    if (pathname.includes('/artist/artistHome')) {
      ...
      setActiveSubTab('adGroup_adGroupManagement')
      setIsArtistMenuOpen(false)
      setIsTechnicMenuOpen(false)
      setIsAdGroupMenuOpen(true)
    }else if (pathname.includes('/adGroup/accesManagement')) {
      setActiveTab('adGroup')
      setActiveSubTab('adGroup_contentAccessManagement')
      setIsArtistMenuOpen(false)
      setIsTechnicMenuOpen(false)
      setIsAdGroupMenuOpen(true)
    } else {
  ...
  useEffect(() => {
    if (user?.graphicThemeName) setUserThemeName(user.graphicThemeName)
  }, [user])

  if (loading || loadingTheme || !user) {
  ...
  const styles = {
    bgActive: theme?.accentHover || '',
    textActive: theme?.textPrimary || '',
          ...
          {canReadAdGroup && (
            <div className="mb-4">
              <div
                className="flex items-center justify-between cursor-pointer rounded-md px-2 py-1"
                onClick={() => {
                  setIsAdGroupMenuOpen(!isAdGroupMenuOpen)
                  handleSelectTab('adGroup')
                  setActiveSubTab('')
                  setIsTechnicMenuOpen(false)
                }}
                onMouseEnter={() => setHoveredButton('adGroup')}
                onMouseLeave={() => setHoveredButton(null)}
                style={
                  activeTab === 'adGroup' || activeSubTab.startsWith('adGroup')
                    ? { backgroundColor: styles.bgActive, color: styles.textActive }
                    : hoveredButton === 'adGroup'
                    ? { backgroundColor: styles.bgHover, color: styles.textHover }
                    : { backgroundColor: styles.bgInactive, color: styles.textInactive }
                }
              >
                ...
                >
                  <UserRoundPen size={20} />
                  {!isCollapsed && gt('adGroupManagement')}
                </Link>
                {!isCollapsed && (isAdGroupMenuOpen ? <ChevronDown size={16} /> : <ChevronRight size={16} />)}
              </div>

              {isAdGroupMenuOpen && !isCollapsed && (
                <div className="ml-6 mt-2 flex flex-col space-y-1 text-sm" style={{ color: styles.textInactive }}>
                  {[
                    { id: 'adGroup_adGroupManagement', label: 'adGroups' },
                    { id: 'adGroup_contentAccessManagement', label: 'contentAccess' }
                  ].map(({ id, label }) => (
                    <SidebarButton
                      key={id}
                      id={id}
                      icon={null}
                      isActive={activeSubTab === id}
                      href={`/${id.replace('_', '/')}`}
